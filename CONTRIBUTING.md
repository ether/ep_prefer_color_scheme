# Contributing to ep_prefer_color_scheme

Thanks for helping improve `ep_prefer_color_scheme` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_prefer_color_scheme` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_prefer_color_scheme from your local clone
pnpm run plugins i --path ../ep_prefer_color_scheme

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_prefer_color_scheme/` are picked up after a server restart.
...
* Etherpad version, ep_prefer_color_scheme version, browser + OS
* Steps to reproduce
